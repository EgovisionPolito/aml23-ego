...
def train(action_classifier, train_loader, val_loader, device, num_classes):
    ...
    for i in range(iteration, training_iterations):
        ...
        end_t = datetime.now()
        logger.info(f"##### TEST ##### - DATA_LOADER_SOURCE: {data_loader_source} | LEN: {len(data_loader_source)} | SOURCE_DATA: {source_data} | LEN: {len(source_data)} | SOURCE_LABEL: {source_label} | LEN: {len(source_label)}")
