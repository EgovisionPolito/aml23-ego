    ...
    def _get_val_indices(self, record, modality):
        ...
        for clip_number in range(self.num_clips):
            if((record_num_frames-clip_radius+2)<=clip_radius):
                logger.info(f"Record_num_frames {record_num_frames}, clip_radius {clip_radius}, (record_num_frames-clip_radius+2)<=clip_radius = {(record_num_frames-clip_radius+2)<=clip_radius}")
                frames_interval = 1
                #clip_radius = (num_frames_per_clip // 2) * frames_interval
